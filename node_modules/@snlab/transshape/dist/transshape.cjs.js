...
function splitEdge (edge) {
  const pointInBetween = d3Interpolate.interpolate(edge[0], edge[1])(0.5);
...
function createInterpolatorNoHoles (from, to, fromOuterRingPrepared, toOuterRingPrepared) {
  const outerRingInterpolator = d3Interpolate.interpolate(fromOuterRingPrepared, toOuterRingPrepared);
...
function createMatchableHoleInterpolators (from, to, numberOfMatchableHoles) {
  ...
  for (let i = 0; i < numberOfMatchableHoles; i++) {
    ...
    const holeInterpolator = d3Interpolate.interpolate(fromHolePrepared, toHolePrepared);
...
function createHoleImploders (polygon, differenceBetweenNumberOfHoles) {
  ...
  for (let i = firstHoleThatNeedsImplodingIndex; i < polygon.coordinates.length; i++) {
    ...
    interpolators.push(d3Interpolate.interpolate(preparedPolygon, preparedImplodePoint));
  }
...
const bisect = d3Array.bisector(d => d.area).left;
...
function collapseTopology (topology, numberOfPieces) {
  ...
  const geojson = topojsonClient.feature(topology, topology.objects.triangles);
  ...
  function mergeSmallestFeature () {
    ...
    const merged = topojsonClient.mergeArcs(topology, [smallest, neighbor]);
...
function cutTriangleInTwo (triangle) {
  ...
  const pointBetweenAB = d3Interpolate.interpolate(a, b)(0.5);
...
function createInterpolator (from, to, preparedFromCoordinates, preparedToCoordinates) {
  const coordinateInterpolator = d3Interpolate.interpolate(preparedFromCoordinates, preparedToCoordinates);
...
function createLineStringInterpolators (fromCoordinates, toCoordinates) {
  ...
  for (let i = 0; i < fromCoordinates.length; i++) {
    ...
    const interpolator = d3Interpolate.interpolate(preparedFromLineString, preparedToLineString);
